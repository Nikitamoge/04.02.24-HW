...
{
	if (pos1 < 1 || Count == 0 || pos1 > Count || pos2 < 1 || pos1 > pos2 || pos2 > 10)
	{
		cout << "Invalid range or empty list!\n";
		return;
	}

	Elem* PrevDel = GetElem(pos1 - 1);
	Elem* Del = GetElem(pos1);
	Elem* newHead = (pos1 == 1) ? GetElem(pos2 + 1) : Head;
	Elem* newTail = (pos2 == Count) ? PrevDel : Tail;
